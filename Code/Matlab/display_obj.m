function display_obj(obj,texture,opacity)
%
...
% 
% x = abs(round(obj.f.vt(:,1)*(sx-1)))+1;
% y = abs(round(obj.f.vt(:,2)*(sy-1)))+1;
% xy = sub2ind([sy sx],y,x);
% texture_pts = xy(texture_idx);
% tval = double(texture_img(texture_pts,:))/255;
tval = double(0.5*ones(14427,1))/255;


% display object 
patch('vertices',obj.v,'faces',obj.f.v,'FaceVertexCData',tval,'FaceColor','black','FaceAlpha',opacity,'edgecolor','none');
shading interp
colormap gray
lighting gouraud;
camproj('perspective');
